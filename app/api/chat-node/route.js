...
export async function POST(req, res) {
    const { language, difficulty, topic, numQuestions, input } = await req.json()

    console.log('----------------------------------------')
    console.log(language, difficulty, topic, numQuestions, input)
    ...
    if(language === 'text'){
        if (input === ''){
            ...
            firstSentence = `Give me ${numQuestions} multiple choice questions based on knowledge from this content: "${input}".`
        }
    }
